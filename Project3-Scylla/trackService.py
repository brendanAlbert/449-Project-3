...
#CQL: functionality not yet working: delete track and edit track

import flask
from flask import g, request, make_response, jsonify, render_template
import uuid
...
# This retrieves a track. This filters the Track table depending on track id
...
def api_filter():
    ...
    if track_id is None:
        ...
        query ="SELECT * FROM track where track_id = {track_id};".format(track_id=track_id)
        track = session.execute(query)
        result = []
        for t in track:
            result.append(t)

        #result = session.execute(query, track_id.bytes_le))
...
def create_track():
    ...
    query ="""
        INSERT INTO track(track_id, track_title, album_title, artist, length_s, url_media)
        VALUES(uuid(), {track_title}, {album_title}, {artist}, {length_s}, {url_media});
        """.format(track_title=track_title, album_title=album_title, artist=artist, length_s=length_seconds, url_media=url_media)
    ...
    location = 'http://localhost:8000/tracks?track_title='+track_title
