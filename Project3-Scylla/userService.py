...
def create_user():
    ...
    query ="""
        ...
        VALUES({username}, {hashed_pw}, {display_name}, {email});
        """.format(username=username, hashed_pw=hashed_pw, display_name=display_name, email=email)

    try:
        session.execute(query)
    except:
        file = open('errorlog.txt', 'a')
        file.write('something went wrong while creating user' + '\n')
        file.close()

    #set up location to be returned in response header
    location = 'http://localhost:8000/users?username=' + username
